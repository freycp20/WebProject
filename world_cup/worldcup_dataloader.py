...
class MatchSchema(Schema):
    ...
    score_home = fields.Int(load_default = None)
    score_away = fields.Int(load_default = None)

    @post_load
    def make_match(self, data, many: bool, partial: bool):
        if many:
            return [Match(d['id'], d['group'], d['date'], 
                        d['stadium'], d['home_team'], d['away_team'], d['score_home'], d['score_away']) \
                            for d in data]
        else:
            d = data
            return Match(d['id'], d['group'], d['date'], 
...
class Match(object):
    def __init__(self, id: int, group: str, date: datetime, stadium: Stadium,
                    home_team: Team, away_team: Team, score_home: int, score_away: int):
        self.id = id
        self.group = group
        self.date = date
